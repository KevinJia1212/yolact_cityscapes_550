...
parser.add_argument('--dataset_dir', default=None, type=str,
                    help='the root of cityscapes dataset')
...
parser.add_argument('--init_from', default='yolact', type=str,
                    help='init weights for training from a yolact weight file with same network or a coco-pretrained yolact weight file.')
...
def train():
    ...
    if args.log:
        ...
        log_name = cfg.name + "_" + time_str  
        log = Log(log_name, args.log_folder, dict(args._get_kwargs()),
        ...
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                if args.log:
                    ...
                    loss_info['T'] = round(sum([loss_avgs[k].get_avg() for k in losses]), precision)
...
def compute_validation_map(epoch, iteration, yolact_net, dataset, log:Log=None):
    with torch.no_grad():
        ...
        all_maps, car_maps, ped_maps, truck_maps, bus_maps, rider_maps = eval_script.evaluate(yolact_net, dataset, train_mode=True)
        end = time.time()

        if log is not None:
            elapsedtime = end - start
            log.log('val_all', all_maps, elapsed=elapsedtime, epoch=epoch, iter=iteration)
            log.log('val_car', car_maps, elapsed=elapsedtime, epoch=epoch, iter=iteration)
            log.log('val_ped', ped_maps, elapsed=elapsedtime, epoch=epoch, iter=iteration)
            log.log('val_truck', truck_maps, elapsed=elapsedtime, epoch=epoch, iter=iteration)
            log.log('val_bus', bus_maps, elapsed=elapsedtime, epoch=epoch, iter=iteration)
            log.log('val_rider', rider_maps, elapsed=elapsedtime, epoch=epoch, iter=iteration)
